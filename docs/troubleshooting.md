...
## 스크롤 피커 중앙 정렬 문제 (v3.3.2 해결)

### 증상
- **선택 오류**: 스크롤 후 정확히 가운데에 오지 않으면 선택되지 않음
- **색상 문제**: 회색으로 색이 변하여 가독성이 떨어짐
- **선택 상태 불일치**: 드래그 중 선택 상태가 정확히 반영되지 않음
...
1. **정확한 중앙 정렬 구현**:
   ```typescript
   // 실시간 중앙 인덱스 계산
   const currentCenterIndex = Math.round(-offset / itemHeight);
...
3. **실시간 선택 상태 관리**:
   ```typescript
   // 실시간 선택 상태 확인
   const isSelected = actualIndex === currentCenterIndex;
   ```
...
### 검증 방법
1. **중앙 정렬 테스트**: 스크롤 후 정확히 중앙에 있는 값이 선택되는지 확인
2. **색상 테스트**: 선택된 값은 오렌지, 선택되지 않은 값은 흰색으로 표시되는지 확인
3. **드래그 테스트**: 드래그 중에도 현재 중앙 값이 시각적으로 표시되는지 확인
4. **일관성 테스트**: 세로/가로 스크롤 피커 모두 동일하게 작동하는지 확인
...
### **원인**
- **동기화 간격 부족**: 5초 간격으로는 미세한 차이 누적
- **단순한 동기화**: 서버 시간만 전송하고 박자 경계 무시
- **임계값 부족**: BPM의 30% 임계값으로는 정밀도 부족
